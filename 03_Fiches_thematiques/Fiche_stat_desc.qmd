...
Parler de janitor, gtsummary, tables, crosstable, ...
...
Certains besoins courants ne sont pas (encore ?) couverts par d'autres packages :
...
Le package est documenté par des vignettes, qui fournissent plus de détails que cette présentation.
...
-   **`tab_pct`** : pour calculer des pourcentages lignes ou colonnes
-   **`tab_mask`** : pour blanchir certaines cellules sous un seuil (de comptage ou de valeur)
-   **`tab_round`** : pour afficher avec le bon niveau d'arrondi et éventuellement gérer la cohérence des sommes à 100 %

**Les fonctions d'affichage**

-   **`tab_render`** : pour afficher le joli tableau → conseil de l'appliquer à chaque fois
-   **`tab_xls`** : pour exporter le joli tableau dans un .xlsx → conseil de ne pas en abuser sauf besoin particulier de repasser par Calc (diffusion, envoi partenaire, etc.)
